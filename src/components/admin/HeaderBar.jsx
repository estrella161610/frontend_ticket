...
import { FaBell, FaUserCircle, FaDove, FaTimes, FaTicketAlt, FaUser, FaBuilding, FaUserFriends } from 'react-icons/fa';
...
const HeaderBar = () => {
  ...
  const { perfil, isLoading: loadingPerfil, error: errorPerfil } = useFetchPerfilAdmin();
        ...
        {isMenuOpen && (
          <div className="absolute left-0 mt-2 bg-white shadow-lg rounded-lg p-4 border border-gray-200 w-64 z-50">
            ...
            <ul className="text-sm mt-2">
              ...
              <li className="p-2 hover:bg-azul hover:text-white rounded cursor-pointer" onClick={() => handleAddItem('usuario', 'Nuevo Usuario')}>Usuario</li>
              <li className="p-2 hover:bg-azul hover:text-white rounded cursor-pointer" onClick={() => handleAddItem('cliente', 'Nuevo cliente')}>Cliente</li>
              <li className="p-2 hover:bg-azul hover:text-white rounded cursor-pointer" onClick={() => handleAddItem('sede', 'Nueva Sede')}>Sedes</li>
              <li className="p-2 hover:bg-azul hover:text-white rounded cursor-pointer" onClick={() => handleAddItem('departamento', 'Nuevo Departamento')}>Departamento</li>
              <li className="p-2 hover:bg-azul hover:text-white rounded cursor-pointer" onClick={() => handleAddItem('grupo', 'Nuevo Grupo')}>Grupo</li>
            </ul>
          </div>
        )}
      </div>
      ...
      <div className="flex ml-4 space-x-2">
        {selectedItems.map(item => (
          ...
          >
            ...
            {item.type === 'usuario' && <FaUser className="mr-1 text-gray-500" />}
            {item.type === 'cliente' && <FaUserFriends className="mr-1 text-gray-500" />}
            {item.type === 'sede' && <FaBuilding className="mr-1 text-gray-500" />}
            {item.type === 'departamento' && <GiOrganigram className="mr-1 text-gray-500" />}
            {item.type === 'grupo' && <MdGroups2 className="mr-1 text-gray-500" />}
              ...
              <div className="flex items-center mb-2">
                ...
                <div className="ml-2">
                  {loadingPerfil ? (
                    ...
                    <>
                      <p className="font-bold text-black text-sm">{perfil.nombre}</p>
