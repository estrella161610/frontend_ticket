...
const TableInicio = ({ tickets, sedes, agentes, clientes, setTicketCount}) => {
  ...
  // Manejo de carga y error del perfil
  if (isLoading) {
  ...
  const filteredTickets = tickets.filter((ticket) => {
    const nombreAgente = agenteMap[ticket.id_agente];
    return perfil && perfil.nombre && nombreAgente === perfil.nombre; // Verificar que perfil y nombre existan
  });

  // Set the ticket count in the parent component
  if (setTicketCount) {
  ...
  const handleRowClick = (ticket) => {
    navigate(`/agente/abrir-ticket/${ticket.id}`, { state: { ticket } });
  };
  ...
  const handleDeleteTickets = async (setTickets) => {
    const ticketsToDelete = filteredTickets.filter((ticket) => checkedTickets[ticket.id]);
    try {
