...
const TableInicio = ({ tickets, sedes, agentes, clientes, setTicketCount }) => {
  ...
  if (isLoading) {
    return <div>Cargando...</div>; // Mostrar un mensaje o un spinner
  }

  if (error) {
      return <div>Error al cargar el perfil: {error.message}</div>; // Manejar el error
  }

  // Filtrar tickets por solicitante
  const filteredTickets = perfil ? tickets.filter((ticket) => {
