...
export const LoginAdmin = () => {
  ...
  const { startAdminLogin } = useAuthStore();
  ...
  useEffect(() => {
    ...
    // console.log("Status de admin", status);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await startAdminLogin({ email, password });
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8">
        ...
        <p className="font-bold text-center text-azul mb-6 text-xl">
          Login Administradores
        </p>
        <form onSubmit={onSubmit} className="w-full max-w-md">
          <div className="form-control w-full max-w-md">
            <label className="label">
              <span className="label-text font-bold mb-1">Correo Electrónico</span>
            </label>
            <input
              type="email"
              id="email"
