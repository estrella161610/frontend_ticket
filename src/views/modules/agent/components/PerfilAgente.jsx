...
import TablaUbicacion from '../../../../components/agent/TablaUbicacion';
import { FaUserCircle, FaTicketAlt} from 'react-icons/fa';
import ChangeContAgent from '../../../../components/ChangeContAgent';
...
const PerfilAgente = () => {
  const { perfil, isLoading, error, handleUpdatePerfil } = useFetchPerfilAgente();
  ...
  //DEPARTAMENTO
  const departamento = departamentos.find(d => d.id === perfil?.id_departamento);
  const nombreDepartamento = departamento ? departamento.nombre : '-';
  //GRUPO
  const grupo = grupos.find(g => g.id === perfil?.id_grupo);
    ...
    <div className="min-h-[80vh] flex bg-gray-100 overflow-hidden">
      <div className="w-2/6 p-4 overflow-y-auto border-r-2 border-gray-200">
        <div className="space-y-4">
          ...
          <div className="bg-white p-2 rounded-md space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-black text-right w-1/3">Departamento</label>
              <input
                type="text"
                className="border-none p-1 rounded text-blue-700 w-1/2 focus:outline-none text-left cursor-pointer"
            ...
            <hr />
            <div className="flex justify-between items-center">
              <label className="text-black text-right w-1/3">Grupo</label>
              <input
                type="text"
                className="border-none p-1 rounded text-blue-700 w-1/2 focus:outline-none text-left cursor-pointer"
                value={nombreGrupo || ""}
      ...
      <ChangeContAgent isOpen={isModalOpen} onClose={handleCloseModal} />
